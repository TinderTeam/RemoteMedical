...
select concat(`report`.`Hs_ID`,`report`.`Pi_SerialNO`) AS `id`,`report`.`Hs_ID` AS `Hs_ID`,`report`.`Pi_SerialNO` AS `Pi_SerialNO`,`report`.`Rs_ReportNO` AS `Rs_ReportNO`,`report`.`Rs_PatientID` AS `Rs_PatientID`,`report`.`Rs_PatientName` AS `Rs_PatientName`,`report`.`Rs_PatientSex` AS `Rs_PatientSex`,`report`.`Rs_PatientAge` AS `Rs_PatientAge`,`report`.`Rs_Modality` AS `Rs_Modality`,`report`.`Rs_Department` AS `Rs_Department`,`report`.`Rs_HospitalNO` AS `Rs_HospitalNO`,`report`.`Rs_BedNO` AS `Rs_BedNO`,`report`.`Rs_BodyPart` AS `Rs_BodyPart`,`report`.`Rs_PreView` AS `Rs_PreView`,`report`.`Rs_Diagnosis` AS `Rs_Diagnosis`,`report`.`Rs_StudyContent` AS `Rs_StudyContent`,`report`.`Rs_StudyConclusion` AS `Rs_StudyConclusion`,`report`.`Rs_ExStudyContent` AS `Rs_ExStudyContent`,`report`.`Rs_ExStudyConclusion` AS `Rs_ExStudyConclusion`,`report`.`Rs_ExStudyContentCode` AS `Rs_ExStudyContentCode`,`report`.`Rs_ExStudyConclusionCode` AS `Rs_ExStudyConclusionCode`,`report`.`Rd_StudyDate` AS `Rd_StudyDate`,`report`.`Rd_ReportDate` AS `Rd_ReportDate`,`report`.`Rd_ReviewDate` AS `Rd_ReviewDate`,`report`.`Rs_OperatorPhysician` AS `Rs_OperatorPhysician`,`report`.`Rs_ReportPhysician` AS `Rs_ReportPhysician`,`report`.`Rs_ReviewPhysician` AS `Rs_ReviewPhysician`,`report`.`Ri_ImageCount` AS `Ri_ImageCount`,`report`.`Rs_ImagePathWL` AS `Rs_ImagePathWL`,`report`.`Rd_ExApply` AS `Rd_ExApply`,`report`.`Rd_ExStartUpImg` AS `Rd_ExStartUpImg`,`report`.`Rd_ExEndUpImg` AS `Rd_ExEndUpImg`,`report`.`Rd_ExReport` AS `Rd_ExReport`,`report`.`Rd_ExDownReport` AS `Rd_ExDownReport`,`report`.`Rs_ExApplyDoctor` AS `Rs_ExApplyDoctor`,`report`.`Rs_ExpertID` AS `Rs_ExpertID`,`report`.`Ri_ExState` AS `Ri_ExState`,`report`.`Ri_ExReportState` AS `Ri_ExReportState`,`expert`.`Es_Name` AS `Es_Name`,`expert`.`Es_PhoneNO` AS `Es_PhoneNO`,`hospital`.`Hs_Name` AS `Hs_Name`,`hospital`.`Hs_ContactsPhone` AS `Hs_ContactsPhone`
from ((`report` join `expert`) join `hospital`)
where ((`report`.`Rs_ExpertID` = `expert`.`Es_ID`) and (`report`.`Hs_ID` = `hospital`.`Hs_ID`));
